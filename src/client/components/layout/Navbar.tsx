...
export function Navbar() {
  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-2000 ease-[cubic-bezier(0.16,1,0.3,1)] ${
        scrolled && !isMenuOpen ? "py-3" : "py-6"
      }`}
    >
      {/* Background Layer */}
      <div
        className={`absolute inset-0 -z-10 transition-all duration-2000 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isMenuOpen
            ? "opacity-0"
            : scrolled
            ? "opacity-100 bg-(--bg-primary)/95 backdrop-blur-md shadow-lg"
            : "opacity-100 bg-linear-to-b from-black/60 to-transparent"
        }`}
      />
